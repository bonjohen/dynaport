...
This module provides a framework-agnostic command-line interface for managing DynaPort
...
def main():
    """DynaPort - Dynamic Port Management for Any Application."""
    pass
...
@click.option("--technology", "-t", help="Technology identifier (e.g., flask, django, fastapi)")
@click.option("--health-check-type", type=click.Choice(['http', 'tcp', 'command', 'custom']), 
              default='http', help="Type of health check to perform")
@click.option("--health-check-command", help="Command to execute for command-based health checks")
...
@click.option("--technology", "-t", help="Filter by technology")
def service_list(json_output: bool, app: Optional[str], technology: Optional[str]):
    ...
    if app:
        ...
        services = registry.get_services_by_technology(technology)
    else:
    ...
    if json_output:
        ...
        for service in services:
            ...
            if service.technology:
                click.echo(f"    Technology: {service.technology}")
...
def service_health(app_id: str, instance: Optional[str]):
    ...
    if service.health_check_type == 'http' and not service.health_endpoint:
...
def adapter_list():
    ...
    # Check for available adapters in the adapters directory
    adapters_dir = Path(__file__).parent.parent / "adapters"
    
    if not adapters_dir.exists():
        click.echo("No adapters directory found")
        return
    
    adapters = []
    
    for file_path in adapters_dir.glob("*_adapter.py"):
        adapter_name = file_path.stem.replace("_adapter", "")
...
def adapter_info(adapter_name: str):
    """Get information about a specific adapter."""
    try:
        # Try to import the adapter module
        module_name = f"dynaport.adapters.{adapter_name}_adapter"
        ...
        for name in dir(adapter_module):
            if name.startswith("DynaPort") and name != "DynaPortAdapter":
                adapter_class = getattr(adapter_module, name)
                break
        
        if not adapter_class:
            click.echo(f"No adapter class found in '{adapter_name}' module", err=True)
            sys.exit(1)
        
        # Display adapter information
        click.echo(f"Adapter: {adapter_name}")
        click.echo(f"Class: {adapter_class.__name__}")
        click.echo(f"Module: {module_name}")
        click.echo(f"Description: {adapter_module.__doc__.strip() if adapter_module.__doc__ else 'No description'}")
        
    except Exception as e:
        click.echo(f"Error getting adapter information: {e}", err=True)
        sys.exit(1)
